...
def forward_kin(q1,q2,q3,q4,q5,q6):
  ...
  Z = []
  d90 = pi/2
  gamma_2 = 78.69 * pi / 180
  gamma_3 = pi/2 - gamma_2
  #   theta, alpha, a, d
  T01 = pose(q1, 0, 0, 0.12675)     # good
  T0g = T01
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
  Z.append(pz)
  T12 = pose(q2 - gamma_2, -d90, 0, 0)   # good
  T0g = T0g* T12
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
  Z.append(pz)
  T23 = pose(q3 - gamma_3, 0, 0.30594, 0)     # good
  T0g = T0g* T23
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
  Z.append(pz)
  T34 = pose(q4, -d90, 0, 0.3)          # good
  T0g = T0g* T34
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
  Z.append(pz)
  T45 = pose(q5, d90, 0, 0)       # good
  T0g = T0g* T45
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
  Z.append(pz)
  T56 = pose(q6, -d90, 0, 0)        # good
  T0g = T0g* T56
  px,py,pz = T0g[0,3], T0g[1,3], T0g[2,3]
  X.append(px)
  Y.append(py)
...
def get_angles(x, y, z, roll, pitch, yaw):
  ...
  alpha, beta, gamma, gamma2, gamma3 = symbols('alpha beta gamma gamma2 gamma3', real = True)
  px, py, pz = symbols('px py pz', real = True)

  # Rotation of joint 3 wrt to the base frame interms the first three angles q1, q2, q3 
  R03 = Matrix([
    [sin(q2 + q3)*cos(q1), cos(q2 + q3)*cos(q1),   -sin(q1)],
...
def main():
        ...
        T = forward_kin(q1,q2,q3,q4,q5,q6)
        # T_eval = T.evalf(subs =  {pi: 3.14});  # 替换为具体数值
        # print(T_eval)
